...
| `share_cart` | Generate a link that opens the cart in Blinkit |
| `check_cart_snapshot` | Return structured cart data for parity checks |
...
| `select_payment_method` | Select COD when available, otherwise generate a UPI QR |
| `pay_now` | Click the final Pay Now button when explicitly enabled |
