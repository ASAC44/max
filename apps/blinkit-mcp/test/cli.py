...
async def main():
    # Initialize Auth
    auth = BlinkitAuth(headless=os.environ.get("HEADLESS", "false").lower() == "true")
    ...
    try:
        ...
        print("  payment          : Select a payment method")
        ...
        while True:
            try:
                ...
                if cmd in ["quit", "exit"]:
                    ...
                    print(
                        "Commands: search <query>, add <index>, cart, checkout, address, payment, login, quit"
                    )
                    ...
                    if args and args[0].isdigit() and int(args[0]) < len(results):
                        await order.add_to_cart(results[int(args[0])]["id"])
                    else:
                        print("Usage: add <item_index>")

                elif cmd == "cart":
                    print(await order.get_cart_items())
                    ...
                    print(await order.select_payment_method())
