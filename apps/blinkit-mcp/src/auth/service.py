...
class BlinkitAuth:
        ...
        try:
            ...
            try:
                ...
                if geolocation:
                    ...
                    print("Location popup detected; manual location is required.")
            except Exception:
    ...
    async def is_logged_in(self) -> bool:
        ...
        try:
            profile = self.page.locator("div[class*='ProfileButton__Container']").first
            try:
                await profile.wait_for(state="visible", timeout=10000)
            except Exception:
                pass

            if await self.page.is_visible("text=My Account") or await self.page.is_visible(
                ".user-profile"
            ):
                return True

            if await profile.is_visible():
                return "login" not in (await profile.inner_text()).lower()
