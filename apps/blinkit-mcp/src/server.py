...
@mcp.tool()
async def share_cart() -> str:
    """Generate a Blinkit share link that can import this cart into another browser or the mobile app."""
    await ctx.ensure_started()
    f = io.StringIO()
    with redirect_stdout(f):
        return await ctx.order.share_cart()


@mcp.tool()
async def check_cart_snapshot() -> str:
    """Return a structured, redacted cart snapshot for Max/Prava parity checks."""
    await ctx.ensure_started()
    f = io.StringIO()
    with redirect_stdout(f):
...
async def select_payment_method():
    ...
    with redirect_stdout(f):
        ...
        if isinstance(res, dict):
            ...
            instruction = "\n\nShow the returned QR image to the user for payment.\n"
...
async def pay_now() -> str:
    """Click Pay Now only when BLINKIT_PAYMENT_SUBMISSION_ENABLED=true after explicit confirmation."""
    await ctx.ensure_started()
    f = io.StringIO()
    with redirect_stdout(f):
