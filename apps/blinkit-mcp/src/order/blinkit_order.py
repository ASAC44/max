...
class BlinkitOrder:
        ...
        try:
            ...
            if "zpaykit" in url or "payment" in url:
                if response.status >= 400:
                    print(f"DEBUG: Payment API error {response.status}.")
                ...
                if "application/json" in response.headers.get("content-type", ""):
                    try:
                        ...
                        if isinstance(data, dict) and (
                            ...
                            print("DEBUG: Payment API failure captured.")
                    except Exception:
                        pass
        except Exception:
            pass
    ...
    async def share_cart(self):
        return await self.cart_service.share_cart()

    async def get_cart_snapshot(self):
        return await self.cart_service.get_cart_snapshot()
