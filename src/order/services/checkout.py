...
class CheckoutService(BaseService):
    async def place_order(self):
        ...
        if await self.page.locator("#payment_widget").is_visible():
            print("Payment page is already open.")
            return
    ...
    async def click_pay_now(self):
        ...
        if os.environ.get("BLINKIT_PAYMENT_SUBMISSION_ENABLED", "false").lower() != "true":
            message = "Payment submission is disabled. Set BLINKIT_PAYMENT_SUBMISSION_ENABLED=true only for an explicitly confirmed attempt."
            print(message)
            return message
        try:
